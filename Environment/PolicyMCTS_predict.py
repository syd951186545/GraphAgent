...
def simulate(node, Policy_net):
    ...
    while not current_state.is_terminal():
        current_state = current_state.get_next_state_with_policy(current_state.node_parent, Policy_net)

    if current_state.current_node == -1:
        state_tup = current_state.state_tup[:-1]
        action_candidate, Qsa_values = Policy_net(state_tup)
        return Qsa_values[0, 0]
    else:
        return 0
...
def Policy_MCTS(node, Policy_net):
    for i in range(config.computation_budget_predict):
